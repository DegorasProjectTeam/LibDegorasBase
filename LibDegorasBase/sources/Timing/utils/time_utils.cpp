                              ...
                              bool utc, bool rm_trailing_zeros)
{
    ...
    if (!tm)
    {
        throw std::runtime_error("[LibDegorasBase,Timing,timePointToString] Error in tm struct.");
    }
...
HRTimePointStd iso8601DatetimeToTimePoint(const std::string& datetime)
{
    ...
    {
        throw std::invalid_argument("[LibDegorasBase,Timing,iso8601DatetimeToTimePoint] Invalid argument: " + datetime);
    }
...
HRTimePointStd julianDateTimeToTimePoint(const JDateTime& jdt)
{
    ...
    {
        std::string submodule("[LibDegorasBase,Timing,julianDateTimeToTimePoint]");
...
HRTimePointStd modifiedJulianDateTimeToTimePoint(const MJDateTime &mjdt)
{
    ...
    if (days_from_epoch < 0)
    {
        std::string submodule("[LibDegorasBase,Timing,modifiedJulianDateTimeToTimePoint]");
...
HRTimePointStd win32TicksToTimePoint(Windows32Ticks ticks)
{
    ...
    if (offset.count() < 0)
    {
        throw std::invalid_argument(
            "[LibDegorasBase,Timing,win32TicksToTimePoint] The ticks represent a time before the Unix epoch.");
    }
...
long long nsDayTohhmmssns(long long ns_in, unsigned int& hour, unsigned int& min, unsigned int& sec, unsigned int& ns)
{
    ...
    auto result = dpbase::math::euclidDiv(ns_in, 86400000000000ll);
...
long double jdtToGmst(long double jdt)
{
    ...
    gmst = dpbase::math::normalizeVal(gmst / 240.L * math::kPi / 180.L, 0.L , 2*math::kPi);
...
long double jdtToLmst(long double jdt, long double lon)
{
    ...
    long double lmst = dpbase::math::normalizeVal(gmst + lon, 0.L, 2*math::kPi);
